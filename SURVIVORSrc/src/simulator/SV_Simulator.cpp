...
std::vector<struct_var_diploid> generate_mutations_diploid(std::string parameter_file, std::map<std::string, std::string> genome) { //EDITED
  parameter par = parse_param(parameter_file);
  ...
  int num_a = 0;
  int num_b = 0;
  std::cout << "generate_mutations_diploid function" << std::endl;
  //duplications
...
void simulate_SV_diploid(std::string ref_file, std::string parameter_file, bool coordinates, std::string output_prefix, int bases_per_snp) { //EDITED
  ...
  if (coordinates || !coordinates) {
    //simulate reads
    svs = generate_mutations_diploid(parameter_file, genomeA); //which genome doesn't matter, just chooses coordinates 
