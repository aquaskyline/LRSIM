...
static inline int my_strncmp(char *s1, char *s2, int l1, int l2){
	int ret;
	if(l1 == l2){
		return strncmp(s1, s2, l1);
	} else if(l1 < l2){
		if((ret = strncmp(s1, s2, l1)) == 0){
			return -1;
		} else return ret;
	} else {
		if((ret = strncmp(s1, s2, l2)) == 0){
			return 1;
		} else return ret;
	}
}

static inline int my_strncasecmp(char *s1, char *s2, int l1, int l2){
	int ret;
	if(l1 == l2){
		return strncasecmp(s1, s2, l1);
	} else if(l1 < l2){
		if((ret = strncasecmp(s1, s2, l1)) == 0){
			return -1;
		} else return ret;
	} else {
		if((ret = strncasecmp(s1, s2, l2)) == 0){
			return 1;
		} else return ret;
	}
}
